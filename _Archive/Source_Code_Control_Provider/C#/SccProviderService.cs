...
namespace Microsoft.Samples.VisualStudio.SourceControlIntegration.SccProvider
{
        ...
        IDisposable 
    {
        ...
        {
            Custom_CheckedOut = _DefaultSCCMonikersListSize,
            Custom_FileUncontrolled = _DefaultSCCMonikersListSize + 1,
        ...
        /// <summary>
        /// Called by the scc manager when the provider is activated. Make visible and enable if necessary scc related menu commands
        /// </summary>
        ...
        /// <summary>
        /// Called by the scc manager when the provider is deactivated. Hides and disable scc related menu commands
        /// </summary>
        ...
        /// <summary>
        /// Called by the scc manager when the user wants to switch to a different source control provider to see if the user needs
        /// to be prompted for closing the current solution (should anything be under the control of this provider)
        /// </summary>
        ...
        public int GetSccGlyph( [In] int cFiles, [In] string[] rgpszFullPaths, [Out] VsStateIcon[] rgsiGlyphs, [Out] uint[] rgdwSccStatus )
        {
            ...
            switch (status)
            {
                ...
                default:
                    ...
                    {
                        // This is an uncontrolled file, display a moniker to indicate that it needs to be added.
                        rgsiGlyphs[0] = (VsStateIcon)(CustomSCCMonikers.Custom_FileUncontrolled);
                        if (rgdwSccStatus != null)
                        {
        ...
        public int GetSccGlyphFromStatus([In] uint dwSccStatus, [Out] VsStateIcon[] psiGlyph)
        {
            switch (dwSccStatus)
            {
                ...
                case (uint) __SccStatus.SCC_STATUS_CONTROLLED:
                    ...
                    break;
                default:
                    psiGlyph[0] = (VsStateIcon)(CustomSCCMonikers.Custom_FileUncontrolled);
                    break;
            }
            return VSConstants.S_OK;
        }
